...
async function getInstalledAppNames(): Promise<void> {
  ...
  const installedApps: AppName[] = []

  for (const installedAppName of allInstalledAppNames) {
    if (installedAppName in apps) {
      installedApps.push(installedAppName as AppName)
    }
  }
